...
@section('links')
    ...
    @endif
    <li class="breadcrumb-item"><a href="#">{{ __('Invoice Detail') }}</a></li>
@endsection
<style type="text/css">
    .button_fix_width {
        width: 135px;
...
@section('multiple-action-button')
    @auth('client')
      @if($invoice->getDueAmount() > 0)
        @if (($currentWorkspace->is_stripe_enabled == 1) ||
            ...
            <div class="text-sm-right mb-1">
                <a href="#" data-toggle="modal" data-size="lg" data-target="#paymentModal"
                    class="btn btn-xs btn-white btn-icon-only width-auto btn btn-primary" >
                    {{ __('Pay Now') }}
                </a>
    ...
    @auth('web')
      @if ($currentWorkspace->creater->id == Auth::user()->id)
        @if($invoice->getDueAmount() > 0)
            <div class="col-sm-auto project_button  py-1 ">
                <a href="#" data-toggle="modal" data-target="#addPaymentModal"  data-size="lg"
                    class="btn btn-xs btn-white btn-icon-only width-auto btn btn-primary button_fix_width" type="button">
                    {{ __('Add Payment') }}
                </a>
            </div>
            @endif
            <div class="col-sm-auto project_button  py-1  ">
                <a href="#" class="btn btn-xs btn-white btn-icon-only width-auto btn btn-primary button_fix_width"
                    data-size="lg" data-ajax-popup="true" data-title="{{ __('Edit Invoice') }}"
                    data-url="{{ route('invoices.edit', [$currentWorkspace->slug, $invoice->id]) }}">
                    {{ __('Edit Invoice') }}
                </a>
            </div>
            <div class="col-sm-auto project_button  py-1  ">
                <a href="#" data-ajax-popup="true" data-title="{{ __('Add Item') }}"
            ...
            <div class="col-sm-auto project_button  py-1  ">
                <a href="#" class="btn btn-xs btn-white btn-icon-only width-auto btn btn-primary cp_link button_fix_width"
                    ...
                    data-toggle="tooltip" data-original-title="{{ __('Click to copy invoice link') }}"><span
                        class="btn-inner--icon text-white"></span><span
                        class="btn-inner--text text-white">{{ __('Copy') }}</span></a>
            </div>
        @endif
    @endauth
    <div class="col-sm-auto project_button  py-1 ">
        <a href="@auth('web') {{ route('invoice.print', [$currentWorkspace->slug, \Illuminate\Support\Facades\Crypt::encryptString($invoice->id)]) }}@elseauth{{ route('client.invoice.print', [$currentWorkspace->slug, \Illuminate\Support\Facades\Crypt::encryptString($invoice->id)]) }} @endauth"
            class="btn btn-xs btn-white btn-icon-only width-auto btn btn-primary button_fix_width">
            {{ __('Print') }}
        </a>
    </div>
@endsection
...
@section('content')
    <div class="row">
        <div class="card" id="printTable">
            <div class="card-body">
                <div class="row ">
                    <div class="col-md-11 invoice-contact">
                ...
                <div class="row invoive-info d-print-inline-flex">
                    <div class="col-sm-4 invoice-client-info">
                        ...
                        @if ($invoice->client)
                            ...
                            @if ($invoice->client->address)
                                <p class="m-0 m-t-10"> {{ $invoice->client->address }}</p>
                            @endif

                            @if ($invoice->client->city)
                                <p class="m-0">{{ $invoice->client->city }},</p>
                            @endif
                            @if ($invoice->client->state)
                                <p class="m-0">{{ $invoice->client->state }},</p>
                            @endif
                            @if ($invoice->client->zipcode)
                                -
                                <p class="m-0">{{ $invoice->client->zipcode }},</p>
                            @endif

                            @if ($invoice->client->country)
                                -
                                <p class="m-0">{{ $invoice->client->country }},</p>
                            @endif
                            <p><a class="text-secondary" href="$" target="_top"><span class="__cf_email__"
                                        data-cfemail="6a0e0f07052a0d070b030644090507">{{ $invoice->client->email }}</span></a>
                            </p>
                            @if ($invoice->client->telephone)
                                -
                                <p class="m-0">{{ $invoice->client->telephone }}.</p>
                            @endif
                        @endif
                    ...
                    <div class="col-sm-4">
                        <h6 class="m-b-20">Invoice Number:
                            <span>{{ App\Models\Utility::invoiceNumberFormat($invoice->invoice_id) }}</span></h6>
                        <h6 class="text-uppercase text-primary">{{ __('Due Amount') }} :
                            <span>{{ $currentWorkspace->priceFormat($invoice->getDueAmount()) }}</span>
                        </h6>
                    </div>
                </div>
                <div class="row">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <h5 class="text-primary">
                                                {{ $currentWorkspace->priceFormat($invoice->getTotal()) }}</h5>
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    ...
                                    <div class="form-icon-user">
                                        ...
                                        <input class="form-control currency_input" type="number" id="amount" name="amount"
                                            value="{{ $invoice->getDueAmount() }}" min="0" step="0.01"
            ...
            <div class="modal fade" id="paymentModal" tabindex="-1" role="dialog" aria-labelledby="paymentModalLabel"
                ...
                <div class="modal-dialog modal-lg" role="document">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <div class="card-box">
                                @if ((isset($currentWorkspace->is_stripe_enabled) && $currentWorkspace->is_stripe_enabled == 1) || (isset($currentWorkspace->is_paypal_enabled) && $currentWorkspace->is_paypal_enabled == 1) || (isset($paymentSetting['is_paystack_enabled']) && $paymentSetting['is_paystack_enabled'] == 'on') || (isset($paymentSetting['is_flutterwave_enabled']) && $paymentSetting['is_flutterwave_enabled'] == 'on') || (isset($paymentSetting['is_razorpay_enabled']) && $paymentSetting['is_razorpay_enabled'] == 'on') || (isset($paymentSetting['is_mercado_enabled']) && $paymentSetting['is_mercado_enabled'] == 'on') || (isset($paymentSetting['is_paytm_enabled']) && $paymentSetting['is_paytm_enabled'] == 'on') || (isset($paymentSetting['is_mollie_enabled']) && $paymentSetting['is_mollie_enabled'] == 'on') || (isset($paymentSetting['is_skrill_enabled']) && $paymentSetting['is_skrill_enabled'] == 'on') || (isset($paymentSetting['is_coingate_enabled']) && $paymentSetting['is_coingate_enabled'] == 'on'))
                                    <ul class="nav nav-tabs bordar_styless py-3">
                                        @if ($currentWorkspace->is_stripe_enabled == 1)
                                            <li>
                                                <a data-toggle="tab" href="#stripe-payment"
                                                    class="active">{{ __('Stripe') }}</a>
                                            </li>
                                        @endif
                                        ...
                                        @if (isset($paymentSetting['is_mercado_enabled']) && $paymentSetting['is_mercado_enabled'] == 'on')
                                            <li>
                                                <a data-toggle="tab" href="#mercado-payment" role="tab" aria-controls="mercado"
                                                    aria-selected="false">{{ __('Mercado Pago') }}</a>
                                            </li>
                                        @endif
                                        @if (isset($paymentSetting['is_paytm_enabled']) && $paymentSetting['is_paytm_enabled'] == 'on')
                                            <li>
                                                <a data-toggle="tab" href="#paytm-payment" role="tab" aria-controls="paytm"
                                        ...
                                        @if (isset($paymentSetting['is_mollie_enabled']) && $paymentSetting['is_mollie_enabled'] == 'on')
                                            <li class="pt-3">
                                                <a data-toggle="tab" href="#mollie-payment" role="tab" aria-controls="mollie"
                                                    aria-selected="false">{{ __('Mollie') }}</a>
                                            </li>
                                        @endif
                                        @if (isset($paymentSetting['is_skrill_enabled']) && $paymentSetting['is_skrill_enabled'] == 'on')
                                            <li class="pt-3">
                                                <a data-toggle="tab" href="#skrill-payment" role="tab" aria-controls="skrill"
                                ...
                                <div class="tab-content mt-3">
                                    @if ($currentWorkspace->is_stripe_enabled == 1)
                                        <div class="tab-pane fade {{ ($currentWorkspace->is_stripe_enabled == 1 && $currentWorkspace->is_paypal_enabled == 1) || $currentWorkspace->is_stripe_enabled == 1 ? 'show active' : '' }}"
                                            ...
                                            <form method="post"
                                                ...
                                                <div class="row">
                                                    ...
                                                    <div class="col-sm-4 text-sm-right mt-3 mt-sm-0">
                                                        <img src="{{ asset('assets/img/payments/master.png') }}" height="24"
                                                            alt="master-card-img">
                                                        <img src="{{ asset('assets/img/payments/discover.png') }}" height="24"
                                                    ...
                                                    <div class="form-group col-md-12">
                                                        ...
                                                        <div class="form-icon-user">
                                                            ...
                                                            <input class="form-control currency_input" required="required"
                                                                ...
                                                                value="{{ $invoice->getDueAmount() }}" min="0" step="0.01"
                                                                max="{{ $invoice->getDueAmount() }}" id="amount">
                                                        </div>
                                                    </div>
                                                </div>
                                                <div class="row">
...
@auth('client')
    @if (($invoice->getDueAmount() > 0 && $currentWorkspace->is_stripe_enabled == 1) ||
        $currentWorkspace->is_paypal_enabled == 1 ||
        (isset($paymentSetting['is_paypal_enabled']) && $paymentSetting['is_paypal_enabled'] == 'on') ||
        (isset($paymentSetting['is_paystack_enabled']) && $paymentSetting['is_paystack_enabled'] == 'on') ||
        (isset($paymentSetting['is_flutterwave_enabled']) && $paymentSetting['is_flutterwave_enabled'] == 'on') ||
